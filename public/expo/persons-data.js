...
(function () {
  ...
  // с инициалов, у остальных — с фамилии, а кое-где его нет вовсе.
  // По сырому ключу Колчак встаёт перед Авксентьевым — список перестаёт
  // быть алфавитным. Инициалы срезаем, пустое добираем из заголовка.
  //
  // Сверено 2026-08-04, после того как обход сочли мёртвым и предложили
  // убрать. Он ЖИВОЙ, ключи с инициалами на месте — все три:
  //   lenin      'в. и. ленин ульянов'
  //   kolchak    'а. в. колчак'
  //   grigorev   'н. а. григорьев серветников'
  // С обходом они встают 38-м, 34-м и 17-м, по алфавиту; без него все
  // трое уезжают в начало списка.
  //
  // ⚠️ Проверка на «остались ли такие ключи» даёт ЛОЖНЫЙ НОЛЬ, если искать
  // заглавные буквы: ключи в нижнем регистре. На это попались обе стороны —
  // и я, и оркестратор. Флаг `i` в регулярке ниже не для красоты.
