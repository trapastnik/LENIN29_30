...
Последний коммит: **`554df2d viewer v3: сменная подложка + per-layer custom + перекраска map_v6 + поиск`** (от 27 мая 2026).

**После него — много несохранённых локальных изменений.** Нужно закоммитить перед началом редактора (см. ниже).
