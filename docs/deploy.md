...
## Как снимать замеры под 4K

Все числа по киоску снимаются так (формулировка зоны `maps`, 2026-08-04):

```
Chrome headless=new через Bash — НЕ браузерная панель,
--window-size=1920,1080 --force-device-scale-factor=2,
собранный dist через npm run kiosk:serve,
замер попадания — elementFromPoint по верху / центру / низу цели.

Режим B для сравнения: --window-size=3840,2160 без флага, DPR 1,
холст 3840×2073 CSS.
```

⚠️ **Браузерная панель для этого не годится** — она держит DPR 2 постоянно,
поэтому «окно 3840» даёт 7680×4320, то есть 8K. Зона `ui` на этом ошиблась
и отозвала две находки из трёх. Зона `maps` попробовала панель один раз
и отбросила замер сама: свёрнутая панель отдала холст 0×0.

🔒 **Замер приводится вместе с режимом** — вьюпорт, DPR, страница (§5).

⚠️ **Проверять цель надо в трёх положениях прокрутки** — начало, середина,
конец. Третий вид негодной тач-цели, найденный `maps` 2026-08-04:
**нажимаемая, но невидимая**. Подъём строк панели до нормы выдавил ряд
кнопок на 579 px ниже границы; стили честные, попадание в верхнем
положении проходит, а до кнопок надо догадаться прокрутить двадцать строк.
