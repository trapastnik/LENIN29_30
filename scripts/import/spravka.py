...
def build_media(media_rows: List[SpravkaRow], scope, aliases, self_target,
    ...
    for i, row in enumerate(media_rows, start=1):
        ...
        # ⚠️ Текст примечания в `media[]` НЕ кладём. Это инструкции верстальщику
        # («Отметить большого бородатого мужика цифрой 1», «Подрезать поля»),
        # и лежать им рядом с `caption_ru` внутри той же записи — значит
        # напрашиваться на отрисовку: поле называется `note_ru`, выглядит как
        # подпись, и однажды кто-то выведет его в карточку. Посетитель музея
        # прочитает указание верстальщику. Нашла зона `maps`, читая мои данные.
        #
        # Сами примечания не теряются: они целиком уходят в `_notes.json`
        # рядом с разделом — все 282 проверены на дубль. В `media[]` остаются
        # только машиночитаемые производные примечания (`directives`,
        # `duplicate_of`): это перечисления, а не проза, и отрисовать их
        # по ошибке нельзя.
        if directives:
