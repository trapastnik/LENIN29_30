...
def rebuild_index(kind: str, entities: List[dict], registry: IdRegistry,
    ...
    for ent in entities:
        ...
        # Социалистическая Федеративная Советская Республика») — им плитку
        # не подписывают.
        #
        # 🔒 Хранится только ПОМЕЧЕННАЯ подпись (`title_locked`, по образцу
        # `xy_locked` у координат Венна). Раньше стояло «записывать, только
        # если пусто», и правило консервировало всё подряд: у семи персон
        # на плитке жило дореформенное написание из подборки M0 («Деникинъ»
        # против «А. И. ДЕНИКИН» в справке), пережившее все пересчёты.
        # Отличить выбранное человеком от затёкшего однажды было нечем —
        # тот же механизм, что держал `camp` Авксентьева зелёным.
        #
        # Непомеченная подпись пересчитывается из справки. Новая сущность
        # получит на плитку полное название — длинно и сразу видно, а значит
        # чинится; молчаливая заморозка не видна вовсе (§13, затычка обязана
        # выглядеть недоделанной). Пометку ставит `seed_index_titles.py`.
        if not rec.get("title_locked") or not rec.get("title_ru"):
