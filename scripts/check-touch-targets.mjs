#!/usr/bin/env node
// Тач-цели: меряем ПОПАДАНИЕ, а не стили.
//
// ЗАЧЕМ. Пороги §1 (основная навигация ≥120, управляющие ≥64) — приёмочный
// параметр, и до сих пор он проверялся глазами. Линтер `design` смотрит
// объявленные стили; попадание пальцем не мерил никто. А стили и попадание
// расходятся в обе стороны: у зоны `simbirsk` кнопка с честным min-height 64
// имела зону нажатия 65 px при норме 120, а `.back-link` в pages.css выглядит
// как 56 px, но добирает зону невидимым ::before до 240. По computed-стилям
// не виден ни первый случай, ни второй.
//
// РЕЖИМ — канон §5, и он печатается в шапке отчёта дословно:
//   Chrome --headless=new через Bash (НЕ панель браузера),
//   --window-size=1920,1080 --force-device-scale-factor=2,
//   собранный dist через npm run kiosk:serve.
// Панель браузера не годится: там document.visibilityState === 'hidden',
// шаги рендеринга не выполняются, ResizeObserver и rAF не доставляются
// вовсе — механизмы, завязанные на кадры, в ней непроверяемы.
//
//   node scripts/check-touch-targets.mjs
//   node scripts/check-touch-targets.mjs --update-baseline
//   node scripts/check-touch-targets.mjs --all      # печатать и законные
//
// Падает НА ПРИРОСТЕ к baseline: известные отклонения записаны там
// с причиной, красным становится только новое (CLAUDE.md §13).

import { spawn } from 'node:child_process';
import { setTimeout as sleep } from 'node:timers/promises';
import { readFile, writeFile, access } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import path from 'node:path';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const BASELINE = path.join(ROOT, 'scripts', 'check-touch-targets.baseline.json');
const CHROME = process.env.CHROME_BIN || '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
// Свой статик-сервер на своём порту — по умолчанию. Полагаться на чужой,
// поднятый заранее, нельзя: он может умереть посреди прогона, и тогда часть
// сцен померится, а часть отдаст chrome-error. Один раз так и вышло: две
// сцены прошли, пять получили страницу ошибки. MTK_BASE_URL переопределяет —
// для отладки на уже запущенном сервере.
// --self-test: снимаем замер ЗАВЕДОМО РАНО и убеждаемся, что признак
// готовности карты это видит. Проба, не поймавшая окно, печатает успех:
// «дефекта нет» и «я не умею мерить» по виду не отличаются (формулировка
// зоны maps). Приём тот же, что подложенный перехват, но с другой стороны —
// там доказываем, что проверка краснеет на дефекте, здесь — что она умеет
// увидеть состояние, в котором дефект возникает.
const САМОПРОВЕРКА = process.argv.includes('--self-test');
const ВНЕШНИЙ = process.env.MTK_BASE_URL || null;
const ПОРТ_СТАТИКИ = 8100 + (process.pid % 600);
const BASE = ВНЕШНИЙ || `http://127.0.0.1:${ПОРТ_СТАТИКИ}`;
const PORT = 9222 + (process.pid % 700);

// Сцены. Раздел показывает не всё сразу: у states.html карточки живут
// за экраном групп, у parties.html список за диаграммой. Меряя только то,
// что открылось само, мы не увидим большую часть целей.
// `минимум` — нижняя граница числа целей, взятая ИЗ ДАННЫХ, а не из головы.
//
// Зачем она, если есть проверка «ноль целей — ошибка». Затем, что ноль ловится,
// а недобор нет: сцена, отрисовавшаяся наполовину, даёт целей больше нуля,
// и счётчик печатает «148/148» — полноту, которой нет. Прогон с четырьмя
// целями вместо тридцати восьми выглядел стопроцентным, и это ровно наш
// главный класс, только внутри самой проверки: выглядит исчерпывающим
// и не является. Нашёл оркестратор на своей машине.
//
// Граница нижняя и мягкая: точное число менялось бы при каждой правке
// вёрстки и сделало бы гейт хрупким. Порядок величины из индекса —
// не меняется.
const SCENES = [
  { имя: 'диаграмма Венна',   url: '/parties.html',        подготовка: null,
    минимум: (d) => d.партий - d.общихСправок },
  { имя: 'список партий',     url: '/parties.html',        подготовка: 'список',
    минимум: (d) => d.партий },
  { имя: 'экран групп',       url: '/states.html',         подготовка: null,
    минимум: () => 6 },
  { имя: 'сетка территорий',  url: '/states.html',         подготовка: 'группа',
    минимум: () => 10 },
  { имя: 'персоналии',        url: '/expo/people.html',    подготовка: null,
    минимум: (d) => d.персон },
  { имя: 'хроника',           url: '/expo/chronicle.html', подготовка: null,
    минимум: () => 6 },
  { имя: 'главная',           url: '/expo/index.html',     подготовка: null,
    минимум: () => 11 },
  // Симбирск двумя проходами. Оглавление — оверлей, и мерить его вместе
  // с лонгридом нельзя: накрытые им цели дают зону в 1 px, а это ошибка
  // измерения, а не дефект. Зона simbirsk напоролась на это первой —
  // её первый замер показал 42 «катастрофы», которых не было.
  // ⚠️ Сцены «карточка территории с картой» здесь НЕТ, и это не забывчивость.
  // Она нужна была, чтобы ловушке готовности <map-unit> было на чём
  // сработать. Довести не удалось: модалка карточки — такой же оверлей,
  // как оглавление Симбирска, и без сужения даёт 16 целей под ней
  // в «перехвачен центр», а сужение до #modal у меня не срабатывает.
  // Оставить полуработающую сцену значило бы записать 16 ложных дефектов
  // в baseline и научиться их не замечать. Долг, а не решение.
  { имя: 'Симбирск',          url: '/simbirsk.html',       подготовка: null,
    минимум: () => 8 },
  { имя: 'Симбирск · оглавление', url: '/simbirsk.html',   подготовка: 'оглавление',
    минимум: () => 8 },
];

/** Числа из контентных индексов — источник нижних границ. */
async function ожидания() {
  const читать = async (rel) => {
    try {
      const j = JSON.parse(await readFile(path.join(ROOT, 'public', 'content', rel), 'utf8'));
      return j.items || (Array.isArray(j) ? j : []);
    } catch { return []; }
  };
  const партии = await читать('parties/_index.json');
  const персоны = await читать('persons/_index.json');
  const территории = await читать('states/_index.json');
  return {
    партий: партии.length,
    общихСправок: партии.filter((i) => i._is_general).length,
    персон: персоны.length,
    территорий: территории.length,
  };
}

const PROBE = (подготовка, безОжиданияКарт) => `(async () => {
  const pause = (ms) => new Promise(r => setTimeout(r, ms));

  // ── ловушка 1: elementFromPoint не спускается в shadow DOM ────────────
  function deepFromPoint(x, y) {
    let el = document.elementFromPoint(x, y);
    while (el && el.shadowRoot) {
      const inner = el.shadowRoot.elementFromPoint(x, y);
      if (!inner || inner === el) break;
      el = inner;
    }
    return el;
  }

  // ── ловушка 2: el.contains() НЕ проходит сквозь границу shadow DOM ────
  // Хост никогда не «содержит» свой внутренний элемент, поэтому наивная
  // проверка помечает перехватом каждую party-card и state-card. В первом
  // прогоне это дало двадцать несуществующих дефектов. Поднимаемся вверх,
  // перепрыгивая с shadow root на его host.
  function попали(el, g) {
    let cur = g;
    while (cur) {
      if (cur === el) return true;
      if (cur.parentElement) { cur = cur.parentElement; continue; }
      const root = cur.getRootNode();
      cur = (root && root.host) ? root.host : null;
    }
    return false;
  }

  const подг = ${JSON.stringify(подготовка)};
  let подготовкаОтчёт = 'не требовалась';
  let корень = document;   // сужается, если сцена меряет оверлей
  if (подг === 'список') {
    const b = [...document.querySelectorAll('button')].find(x => /Список|List/.test(x.textContent));
    if (b) { b.click(); подготовкаОтчёт = 'нажат «Список»'; await pause(900); }
    else подготовкаОтчёт = 'НЕ УДАЛАСЬ: кнопка «Список» не найдена';
  } else if (подг === 'оглавление') {
    const b = document.getElementById('toc-btn')
      || [...document.querySelectorAll('button')].find(x => /оглавлен|contents/i.test(x.textContent + (x.title || '')));
    if (b) {
      b.click(); await pause(900);
      // ── ловушка 7: оверлей накрывает цели, которые он не отменяет ──────
      // Меряя открытый оверлей вместе со страницей под ним, получаем зону
      // в 1 px у всего, что он накрыл. Зона simbirsk напоролась первой:
      // её первый замер показал 42 «катастрофы», которых не было. Цели под
      // оверлеем не исчезли — они просто меряются в СОСЕДНЕЙ сцене, где
      // оверлей закрыт. Поэтому здесь сужаем сбор до самого оверлея.
      const lv = document.querySelector('longread-view');
      const toc = lv && lv.shadowRoot && lv.shadowRoot.querySelector('.toc');
      if (toc && !toc.hasAttribute('hidden')) {
        корень = toc;
        подготовкаОтчёт = 'открыто оглавление, замер сужен до него';
      } else {
        подготовкаОтчёт = 'НЕ УДАЛАСЬ: оглавление не раскрылось';
      }
    }
    else подготовкаОтчёт = 'НЕ УДАЛАСЬ: кнопка оглавления не найдена';
  } else if (подг === 'карта') {
    // Территория с картой сегодня одна на 59, и в какой она группе — знать
    // не хотим: перебираем группы, пока не найдётся строка .has-map.
    // Иначе сцена ломалась бы от любой перестановки групп.
    const группы = [...document.querySelectorAll('.group-btn')];
    подготовкаОтчёт = 'НЕ УДАЛАСЬ: территория с картой не найдена ни в одной группе';
    for (const g of группы) {
      g.click(); await pause(900);
      const строка = document.querySelector('.state-plate.has-map');
      if (строка) {
        строка.click(); await pause(1800);
        // Модалка карточки — такой же оверлей, как оглавление Симбирска:
        // без сужения 16 целей под ней уходят в «перехвачен центр». Ловушка
        // оказалась не про оглавление, а про любой оверлей — и я наступил
        // на неё второй раз, уже починив первый случай.
        const modal = document.getElementById('modal');
        if (modal && !modal.classList.contains('hidden')) корень = modal;
        подготовкаОтчёт = 'открыта карточка с картой из «'
          + g.textContent.replace(/\s+/g, ' ').trim().slice(0, 24) + '», замер сужен до модалки';
        break;
      }
      const назад = document.querySelector('#group-detail .back-link, #group-detail button');
      if (назад) { назад.click(); await pause(500); }
    }
  } else if (подг === 'группа') {
    const b = [...document.querySelectorAll('button, [role=button]')]
      .find(x => /Красные|Белые|Зелёные|Национальные|Интервен|Революционная/i.test(x.textContent));
    if (b) { b.click(); подготовкаОтчёт = 'открыта «' + b.textContent.replace(/\\s+/g,' ').trim().slice(0,26) + '»'; await pause(1400); }
    else подготовкаОтчёт = 'НЕ УДАЛАСЬ: плитка группы не найдена';
  }

  // ── ловушка 8: <map-unit> до первой доставки ResizeObserver ───────────
  // Видимый размер карте задаёт panZoom.reset(), а зовётся он ТОЛЬКО из
  // ResizeObserver — синхронного вызова нет. До первой доставки карта
  // нарисована в натуральную величину своего viewBox, и одно и то же
  // состояние даёт два противоположных ложных диагноза: komuch (1500×1000)
  // выглядит переполнением, simbirsk-july (31×33) — схлопнутым в точку,
  // неотличимо от поломки. Зона simbirsk едва не отправила maps ложную
  // регрессию именно так.
  //
  // Признак готовности — НЕНУЛЕВОЙ transform у #container, не таймаут:
  // таймаут отвечает «прошло столько-то», а не «карта готова».
  // Не дождались — сцена несостоявшаяся, а не «померена как есть»:
  // ложное красное тут дороже пропуска, чинить его будет тот, кто
  // про карту ничего не знает.
  let картыОтчёт = 'карт нет';
  {
    // Со спуском в shadow DOM: карта монтируется внутри state-card,
    // а в light DOM её нет — искать там значит не найти никогда.
    const карты = [];
    (function обход(root) {
      for (const el of root.querySelectorAll('*')) {
        if (el.tagName === 'MAP-UNIT') карты.push(el);
        if (el.shadowRoot) обход(el.shadowRoot);
      }
    })(document);
    if (карты.length) {
      const готова = (m) => {
        const c = m.shadowRoot && m.shadowRoot.getElementById('container');
        if (!c) return false;
        const t = getComputedStyle(c).transform;
        return !!t && t !== 'none' && !/matrix\(1,\s*0,\s*0,\s*1,\s*0,\s*0\)/.test(t);
      };
      let ждали = 0;
      const ждать = !${безОжиданияКарт};
      while (ждать && ждали < 6000 && !карты.every(готова)) { await pause(120); ждали += 120; }
      const сколько = карты.filter(готова).length;
      картыОтчёт = сколько === карты.length
        ? 'карт ' + карты.length + ', все готовы за ' + ждали + ' мс'
        : 'НЕ ГОТОВЫ: ' + сколько + ' из ' + карты.length + ' за ' + ждали + ' мс';
    }
  }

  // Кто на самом деле прокручивается. У разделов документ не прокручивается
  // вовсе (main с overflow hidden), скроллер — внутренний контейнер.
  function найтиСкроллер() {
    const кандидаты = [];
    const обход = (root) => {
      for (const el of root.querySelectorAll('*')) {
        if (el.shadowRoot) обход(el.shadowRoot);
        if (!/auto|scroll/.test(getComputedStyle(el).overflowY)) continue;
        if (el.scrollHeight - el.clientHeight < 4) continue;
        кандидаты.push(el);
      }
    };
    обход(document);
    const d = document.scrollingElement;
    if (d && d.scrollHeight - d.clientHeight >= 4) кандидаты.unshift(d);
    return кандидаты[0] || null;
  }

  const скроллер = найтиСкроллер();
  const ход = скроллер ? скроллер.scrollHeight - скроллер.clientHeight : 0;

  // ── ловушка 3: три положения прокрутки не покрывают список ────────────
  // При ходе 3203 px между «началом» и «серединой» остаётся полоса, в которую
  // цель не попадает ни разу, — и она ложно окажется недоступной. В первом
  // прогоне так «пропали» 24 карточки персон. Шагаем на 0.8 окна.
  const шаг = Math.max(1, Math.round(innerHeight * 0.8));
  const позиции = ход < 4 ? [0] : (() => {
    const out = [];
    for (let y = 0, i = 0; i < 40; i++, y += шаг) { out.push(Math.min(y, ход)); if (y >= ход) break; }
    return out;
  })();

  const СЕЛЕКТОР = 'button, a[href], [role="button"], .chip, .card, input, select, state-card, party-card';

  function собрать() {
    const acc = [];
    const обход = (root, путь) => {
      for (const el of root.querySelectorAll('*')) {
        if (el.shadowRoot) обход(el.shadowRoot, (путь ? путь + ' › ' : '') + el.tagName.toLowerCase());
        if (!el.matches(СЕЛЕКТОР)) continue;
        const b = el.getBoundingClientRect();
        if (b.width < 1 || b.height < 1) continue;
        acc.push({ el, путь: путь || 'страница', box: b });
      }
    };
    обход(корень === document ? document : корень, '');
    return acc;
  }

  const подпись = (el) => ((el.getAttribute('aria-label') || el.textContent || '')
      .replace(/\\s+/g, ' ').trim().slice(0, 40)) || ('<' + el.tagName.toLowerCase() + '>');

  // ── контраст текста против ФАКТИЧЕСКОГО фона ──────────────────────────
  // Статикой этот класс не берётся по устройству: фон стоит в другом правиле
  // и часто даже не в предке по селектору — вложенность идёт через разметку.
  // Зона design проверила заранее, поймал бы её линтер три её же ошибки
  // (1.45, 1.08, 1.29) — не поймал бы ни одной. Здесь мы стоим на отрисованной
  // странице и видим ту пару, которая сложилась на самом деле.
  function разбор(c) {
    const m = String(c).match(/rgba?\\(([^)]+)\\)/);
    if (!m) return null;
    const p = m[1].split(',').map(s => parseFloat(s));
    return { r: p[0], g: p[1], b: p[2], a: p.length > 3 ? p[3] : 1 };
  }
  function поверх(верх, низ) {
    const a = верх.a;
    return { r: верх.r * a + низ.r * (1 - a), g: верх.g * a + низ.g * (1 - a),
             b: верх.b * a + низ.b * (1 - a), a: 1 };
  }
  function яркость(c) {
    const f = (v) => { v /= 255; return v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4); };
    return 0.2126 * f(c.r) + 0.7152 * f(c.g) + 0.0722 * f(c.b);
  }
  /**
   * Фон, который реально под элементом: копим слои, пока не станет непрозрачно.
   *
   * ⚠️ Градиент или картинка обрывают замер. У них backgroundColor прозрачен,
   * и наивный подъём уходит выше — к фону сцены. На главной это дало
   * «Симбирскъ 1918–1919: контраст 1.03, чёрное на чёрном», хотя плитка
   * там светло-серая и текст читается: я взял тёмное дерево страницы вместо
   * заливки плитки. Считать средний цвет градиента нельзя — текст может
   * лежать на любой его точке. Поэтому честно возвращаем «не измерено»:
   * ложный дефект хуже неизмеренного, он уводит чинить то, что цело.
   */
  // Цветовые остановки градиента. Текст может лежать на любой его точке,
  // поэтому берём их все и судим по худшей — консервативно и честно.
  // Без этого проверка молчала бы на всей бумажной вёрстке: карточки персон,
  // плитки главной и сцена нарисованы градиентами, и «не измерено» съело бы
  // больше половины текстов, оставшись при этом незаметным в отчёте.
  function стопыГрадиента(bgImage) {
    if (!/gradient\\(/.test(bgImage)) return null;      // url(...) — не разобрать
    const цвета = bgImage.match(/rgba?\\([^)]+\\)/g) || [];
    const стопы = цвета.map(разбор).filter(c => c && c.a > 0.05);
    return стопы.length ? стопы : null;
  }

  function фонПод(el) {
    let слои = [];
    let cur = el;
    let достоверно = true;
    let варианты = null;          // если под текстом градиент — набор его стопов
    while (cur) {
      const cs = getComputedStyle(cur);
      if (cs.backgroundImage && cs.backgroundImage !== 'none') {
        const стопы = стопыГрадиента(cs.backgroundImage);
        if (стопы) { варианты = стопы; break; }
        достоверно = false;       // картинка: цвет под текстом неизвестен
        break;
      }
      const bg = разбор(cs.backgroundColor);
      if (bg && bg.a > 0) {
        слои.push(bg);
        if (bg.a >= 0.999) break;
      }
      cur = cur.parentElement || (cur.getRootNode() && cur.getRootNode().host) || null;
    }
    if (варианты) {
      // Слои над градиентом накладываем на каждый его стоп.
      const итоги = варианты.map(стоп => {
        let c = { ...стоп, a: 1 };
        for (let i = слои.length - 1; i >= 0; i--) c = поверх(слои[i], c);
        return c;
      });
      итоги.достоверно = true;
      итоги.градиент = true;
      return итоги;
    }
    // Ниже всего — белый лист: если непрозрачного фона так и не нашлось,
    // страница ляжет на подложку браузера.
    let итог = { r: 255, g: 255, b: 255, a: 1 };
    for (let i = слои.length - 1; i >= 0; i--) итог = поверх(слои[i], итог);
    итог.достоверно = достоверно;
    return итог;
  }
  function контраст(el) {
    const cs = getComputedStyle(el);
    const текст = разбор(cs.color);
    if (!текст || текст.a < 0.05) return null;
    const фон = фонПод(el);
    if (!фон.достоверно) return null;   // под текстом картинка: цвет неизвестен
    // Градиент даёт набор точек — судим по ХУДШЕЙ: текст может лежать на любой.
    const точки = Array.isArray(фон) ? фон : [фон];
    let отн = Infinity, худший = точки[0];
    for (const т of точки) {
      const цвет = текст.a < 0.999 ? поверх(текст, т) : текст;
      const о = (Math.max(яркость(цвет), яркость(т)) + 0.05)
              / (Math.min(яркость(цвет), яркость(т)) + 0.05);
      if (о < отн) { отн = о; худший = т; }
    }
    // Порог WCAG AA: 3.0 для крупного текста (≥24px, либо ≥18.66px жирным),
    // 4.5 для остального. Кегль берём фактический, а не заявленный.
    const px = parseFloat(cs.fontSize) || 16;
    const жирный = (parseInt(cs.fontWeight) || 400) >= 700;
    const крупный = px >= 24 || (px >= 18.66 && жирный);
    return { знач: Math.round(отн * 100) / 100, порог: крупный ? 3 : 4.5, кегль: Math.round(px),
             фонЦвет: 'rgb(' + [худший.r, худший.g, худший.b].map(Math.round).join(', ') + ')',
             градиент: !!фон.градиент };
  }

  // Пороги §1. Чипы Венна — записанное исключение: меряем, но не судим.
  function порог(el, путь) {
    if (el.classList.contains('chip') && /venn/.test(путь)) return { need: null, роль: 'чип Венна' };
    if (el.classList.contains('back-link')) return { need: 120, роль: 'основная навигация' };
    if (/^(STATE-CARD|PARTY-CARD)$/.test(el.tagName)) return { need: 64, роль: 'карточка справки' };
    return { need: 64, роль: 'управляющий' };
  }

  // ── ловушка 4: ключ сводки ────────────────────────────────────────────
  // Ни подпись, ни порядковый номер не годятся. Подпись схлопнет 33 карточки
  // с пустым текстом и одинаковым боксом в одну строку (отчёт покажет 12
  // целей вместо 43). Порядковый номер разъедется там, где состав DOM меняется
  // при прокрутке — у персон и хроники отрисовка ленивая, и один и тот же
  // индекс в двух положениях указывает на РАЗНЫЕ карточки, отчего «лучшее
  // состояние» склеивается из кусков двух целей. Ключ — сама ссылка на узел.
  const итог = new Map();

  function замерить(el, путь, box) {
    const th = порог(el, путь);
    const cx = box.left + box.width / 2;
    const вКадреX = cx >= 0 && cx <= innerWidth;
    const точки = [box.top + 2, box.top + box.height / 2, box.bottom - 2].map(py => {
      if (!вКадреX || py < 0 || py > innerHeight) return 'вне';
      return попали(el, deepFromPoint(cx, py)) ? 'да' : 'перехват';
    });

    let up = 0, down = 0;
    const cy = box.top + box.height / 2;
    if (вКадреX && cy >= 0 && cy <= innerHeight) {
      const ok = (py) => py >= 0 && py <= innerHeight && попали(el, deepFromPoint(cx, py));
      while (up < 600 && ok(cy - up - 1)) up++;
      while (down < 600 && ok(cy + down + 1)) down++;
    }
    // +1 — сам центр: шаги считаются ОТ него, иначе 64 px дают 63
    // и каждая цель ложно оказывается на пиксель ниже нормы.
    const хит = (up + down) ? up + down + 1 : 0;
    const оценка = (точки.every(t => t === 'да') ? 4 : 0)
                 + (точки[1] === 'да' ? 2 : 0)
                 + (точки.includes('да') ? 1 : 0);
    return {
      подпись: подпись(el), путь, роль: th.роль, порог: th.need,
      бокс: [Math.round(box.width), Math.round(box.height)],
      хит, точки, оценка,
    };
  }

  const запомнить = (el, зам) => {
    const было = итог.get(el);
    if (!было || зам.оценка > было.оценка || (зам.оценка === было.оценка && зам.хит > было.хит)) итог.set(el, зам);
  };

  for (const y of позиции) {
    if (скроллер) { скроллер.scrollTop = y; await pause(300); }
    for (const { el, путь, box } of собрать()) запомнить(el, замерить(el, путь, box));
  }

  // ── ловушка 5: дискретные положения — тоже выборка ────────────────────
  // Шаг в 0.8 окна покрывает список, но цель может каждый раз останавливаться
  // под закреплённой шапкой и ни в одном положении не оказаться свободной.
  // В хронике так «пропали» две кнопки «Справка →», которые на деле
  // доступны — надо лишь прокрутить к ним. Доводим каждую спорную цель
  // до центра кадра и меряем ещё раз: это ровно то, что делает посетитель.
  //
  // ── ловушка 6: цель во ВЛОЖЕННОМ скролл-контейнере ────────────────────
  // Прокручивается не только страница. У цели, лежащей в своём скролл-блоке
  // (оглавление, список фильтров, лента), край может быть срезан границей
  // ЭТОГО блока, а не окна: зона выходит 68 при норме 120, хотя прокрутка
  // самого блока делает цель целой. Нашла зона simbirsk на своём оглавлении;
  // у меня такие блоки есть в хронике и персоналиях, просто пока не выстрелили.
  // Поэтому в спорные берём не только цели с недоступным центром, но и те,
  // у кого зона МЕНЬШЕ ПОРОГА: scrollIntoView прокручивает все контейнеры
  // по цепочке, включая вложенные, — и второй замер показывает настоящую зону.
  const спорные = [...итог.entries()].filter(([, з]) =>
    з.точки[1] !== 'да' || (з.порог != null && з.хит > 0 && з.хит < з.порог));
  for (const [el] of спорные) {
    try { el.scrollIntoView({ block: 'center' }); } catch { continue; }
    await pause(260);
    const b = el.getBoundingClientRect();
    if (b.width < 1 || b.height < 1) continue;
    запомнить(el, замерить(el, итог.get(el).путь, b));
  }

  // Контраст меряем не только у целей: подписи, заголовки и пояснения
  // читаются так же, а провал у них так же незаметен в коде. Берём
  // элементы с СОБСТВЕННЫМ текстом — иначе один и тот же текст посчитается
  // столько раз, сколько над ним обёрток.
  const контрасты = [];
  {
    const виден = (el, b) => b.width >= 1 && b.height >= 1
      && b.bottom > 0 && b.top < innerHeight
      && getComputedStyle(el).visibility !== 'hidden';
    const обход = (root, путь) => {
      for (const el of root.querySelectorAll('*')) {
        if (el.shadowRoot) обход(el.shadowRoot, (путь ? путь + ' › ' : '') + el.tagName.toLowerCase());
        const свой = [...el.childNodes].some(n => n.nodeType === 3 && n.textContent.trim().length > 1);
        if (!свой) continue;
        const b = el.getBoundingClientRect();
        if (!виден(el, b)) continue;
        const k = контраст(el);
        if (!k || k.знач >= k.порог) continue;
        контрасты.push({
          текст: (el.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 44),
          путь: путь || 'страница',
          отношение: k.знач, порог: k.порог, кегль: k.кегль,
          цвет: getComputedStyle(el).color,
          фон: k.фонЦвет + (k.градиент ? ' (худшая точка градиента)' : ''),
        });
      }
    };
    обход(корень === document ? document : корень, '');
  }

  return JSON.stringify({
    цели: [...итог.values()],
    контрасты,
    // Адрес и признак служебной страницы Chrome. Без этого гейт мерит
    // экран «не удалось открыть» как обычную сцену: там есть кнопки
    // Reload и Details, целей больше нуля, и проверка на пустой вход
    // молчит. Один раз так и вышло — три кнопки страницы ошибки уехали
    // в baseline как известные дефекты «главной».
    адрес: location.href,
    рано: window.__рано || null,
    страницаОшибки: !!document.querySelector('#main-frame-error, .error-code')
                    || /chrome-error/.test(location.protocol),
    режим: {
      css: [innerWidth, innerHeight], dpr: devicePixelRatio,
      физически: [innerWidth * devicePixelRatio, innerHeight * devicePixelRatio],
      uiScale: getComputedStyle(document.documentElement).getPropertyValue('--ui-scale').trim(),
      шрифты: document.fonts.status, видимость: document.visibilityState,
      подготовка: подготовкаОтчёт,
      карты: картыОтчёт,
      скроллер: скроллер ? (скроллер.id || скроллер.className || скроллер.tagName) : 'нет прокрутки',
      ход, положений: позиции.length,
    },
  });
})()`;

/**
 * Классификация. Правилом, а не базой: 29 повёрнутых карточек персон дают
 * перехват углом, и записывать их в baseline поимённо — значит забить её
 * законным и перестать её читать (§13 про базу, которая не читается).
 */
function разобрать(ц) {
  const центр = ц.точки[1];
  // «Вне кадра» и «перехват» — разные диагнозы, и путать их нельзя.
  // Цель, до которой доскроллили и она всё равно вне кадра, физически
  // недостижима; перехваченная — достижима глазами, но не пальцем.
  if (центр === 'вне' && !ц.точки.includes('да'))
    return { дефект: true, вид: 'не попадает в кадр даже после прокрутки к ней' };
  if (центр === 'перехват') return { дефект: true, вид: 'перехвачен ЦЕНТР цели' };
  if (центр === 'вне')
    return { дефект: false, вид: 'центр за кромкой в замеренных положениях, край доступен' };
  if (ц.порог != null && ц.хит > 0 && ц.хит < ц.порог)
    return { дефект: true, вид: `хит-зона ${ц.хит} при норме ${ц.порог}` };
  if (ц.точки.includes('перехват'))
    return { дефект: false, вид: 'перехват по краю при доступном центре — соседний элемент заходит углом' };
  if (ц.точки.includes('вне'))
    return { дефект: false, вид: 'край за кромкой кадра при доступном центре' };
  return null;
}

async function cdp(ws, method, params = {}) {
  const id = cdp._id = (cdp._id || 0) + 1;
  return new Promise((res, rej) => {
    const on = (ev) => {
      const m = JSON.parse(ev.data);
      if (m.id !== id) return;
      ws.removeEventListener('message', on);
      m.error ? rej(new Error(m.error.message)) : res(m.result);
    };
    ws.addEventListener('message', on);
    ws.send(JSON.stringify({ id, method, params }));
  });
}

async function мерить(сцена) {
  const url = BASE + сцена.url;
  const tab = await (await fetch(`http://127.0.0.1:${PORT}/json/new?${encodeURIComponent(url)}`, { method: 'PUT' })).json();
  const ws = new WebSocket(tab.webSocketDebuggerUrl);
  await new Promise((r, j) => { ws.onopen = r; ws.onerror = j; });
  await cdp(ws, 'Page.enable');
  if (САМОПРОВЕРКА) {
    // Пауза перед замером окна не ловит: ResizeObserver доставляется
    // в первом же кадре после вставки, и 250 мс — уже поздно. Снимаем
    // состояние В МОМЕНТ появления map-unit, наблюдателем, поставленным
    // до загрузки документа. Иначе самопроверка доказывала бы только то,
    // что мы ждём достаточно долго, — а не то, что признак различает
    // готовое и неготовое.
    await cdp(ws, 'Page.addScriptToEvaluateOnNewDocument', { source: `
      window.__рано = null;
      new MutationObserver((recs) => {
        for (const r of recs) for (const n of r.addedNodes) {
          if (n.nodeType !== 1) continue;
          const m = n.tagName === 'MAP-UNIT' ? n : (n.querySelector && n.querySelector('map-unit'));
          if (!m || window.__рано) continue;
          const c = m.shadowRoot && m.shadowRoot.getElementById('container');
          const t = c ? getComputedStyle(c).transform : 'нет контейнера';
          window.__рано = { transform: t,
            готова: !!t && t !== 'none' && !/matrix\(1,\s*0,\s*0,\s*1,\s*0,\s*0\)/.test(t) };
        }
      }).observe(document.documentElement, { childList: true, subtree: true });
    ` });
  }
  // ⚠️ --window-size задаёт ОКНО, а не холст. В headless это молча съедает
  // 87 px высоты: при --window-size=1920,1080 вьюпорт выходит 1920×993.
  // Восемь процентов высоты — не мелочь: у диаграммы Венна на них меняется
  // весь кадр (1425×801 против 1580×888), а с ним и расстояния между целями.
  // Точный холст задаём эмуляцией, флаг окна оставляем — он влияет на то,
  // каким Chrome считает экран.
  await cdp(ws, 'Emulation.setDeviceMetricsOverride', {
    width: 1920, height: 1080, deviceScaleFactor: 2, mobile: false,
  });
  // Ждём отрисовку и — обязательно — шрифты: до fonts.ready строка нарисована
  // запасным начертанием и уже настоящей, а от ширины подписи зависят размеры.
  await sleep(САМОПРОВЕРКА ? 250 : 3400);
  const { result, exceptionDetails } = await cdp(ws, 'Runtime.evaluate', {
    expression: PROBE(сцена.подготовка, САМОПРОВЕРКА), awaitPromise: true, returnByValue: true,
  });
  ws.close();
  await fetch(`http://127.0.0.1:${PORT}/json/close/${tab.id}`).catch(() => {});
  if (exceptionDetails) throw new Error(exceptionDetails.exception?.description || exceptionDetails.text);
  return JSON.parse(result.value);
}

const ключ = (сцена, ц, вид) => `${сцена} | ${ц.роль} | ${ц.подпись} | ${вид}`;

async function main() {
  const обновить = process.argv.includes('--update-baseline');
  const всё = process.argv.includes('--all');

  // Статику поднимаем сами, если её нет. Гейт, требующий заранее запущенного
  // сервера, в `npm run check` не поставишь, а именно там он и нужен.
  let свой = null;
  const живой = async () => {
    try { return (await fetch(BASE + '/expo/index.html')).ok; } catch { return false; }
  };

  if (!await живой()) {
    if (ВНЕШНИЙ) {
      console.error(`✗ ${ВНЕШНИЙ} не отвечает. Подними сервер или убери MTK_BASE_URL — тогда подниму свой.`);
      process.exit(1);
    }
    свой = spawn('python3', ['-m', 'http.server', String(ПОРТ_СТАТИКИ), '--bind', '127.0.0.1',
                             '--directory', path.join(ROOT, 'dist')], { stdio: 'ignore' });
    for (let i = 0; i < 40; i++) { if (await живой()) break; await sleep(250); }
    if (!await живой()) {
      console.error(`✗ не удалось поднять статику на ${BASE}. Проверь, что dist/ собран: npm run build`);
      try { свой.kill(); } catch {}
      process.exit(1);
    }
  }
  const стопСервер = () => { if (свой) { try { свой.kill(); } catch {} свой = null; } };
  process.on('exit', стопСервер);

  const chrome = spawn(CHROME, [
    '--headless=new', `--remote-debugging-port=${PORT}`,
    '--window-size=1920,1080', '--force-device-scale-factor=2',
    '--hide-scrollbars', '--no-first-run',
    '--user-data-dir=' + (process.env.TMPDIR || '/tmp') + 'mtk29-touch',
  ], { stdio: 'ignore' });
  const убить = () => { try { chrome.kill(); } catch {} };
  process.on('exit', убить);

  let поднялся = false;
  for (let i = 0; i < 80; i++) {
    try { await fetch(`http://127.0.0.1:${PORT}/json/version`); поднялся = true; break; }
    catch { await sleep(250); }
  }
  if (!поднялся) { console.error(`✗ Chrome не поднялся. Проверь путь: ${CHROME}`); убить(); стопСервер(); process.exit(1); }

  // База хранит ЧИСЛО по каждому ключу, а не просто наличие. Три кнопки
  // «Справка» в групповых блоках Венна неотличимы по подписи и роли, и без
  // счётчика четвёртая такая же прошла бы молча — гейт видел бы знакомый
  // ключ. Тот же механизм, что пофайловые счётчики у brand:lint.
  let база = [];
  try { база = JSON.parse(await readFile(BASELINE, 'utf8')).known || []; } catch {}
  const известно = new Map(база.map(b => [b.ключ, b.число ?? 1]));

  const ожид = await ожидания();
  const находки = [];
  const законные = [];
  const пустые = [];
  const недобор = [];
  const контрасты = [];
  const сСостоянием = [];
  let целейВсего = 0, сценОК = 0;

  console.log('режим: Chrome --headless=new через Bash (НЕ панель браузера),');
  console.log('       --window-size=1920,1080 --force-device-scale-factor=2,');
  console.log(`       собранный dist через npm run kiosk:serve (${BASE}),`);
  console.log('       замер попадания — elementFromPoint по верху/центру/низу');
  console.log('       со спуском в shadow DOM, прокрутка шагом 0.8 окна.\n');

  for (const сцена of SCENES) {
    // Статику проверяем перед КАЖДОЙ сценой. python3 -m http.server
    // однопоточен и за прогон отдаёт сотни файлов; один раз он отвалился
    // на последней сцене, и она пришла как chrome-error. Проверка живости
    // в начале прогона этого не покрывает.
    if (!await живой() && !ВНЕШНИЙ) {
      стопСервер();
      свой = spawn('python3', ['-m', 'http.server', String(ПОРТ_СТАТИКИ), '--bind', '127.0.0.1',
                               '--directory', path.join(ROOT, 'dist')], { stdio: 'ignore' });
      for (let i = 0; i < 40; i++) { if (await живой()) break; await sleep(250); }
    }

    let res;
    try { res = await мерить(сцена); }
    catch (e) { console.error(`  ✗ ${сцена.имя}: ${String(e).slice(0, 140)}`); пустые.push(сцена.имя); continue; }

    const m = res.режим;
    const n = res.цели.length;
    целейВсего += n;
    сСостоянием.push({ сцена: сцена.имя, карты: m.карты, рано: res.рано });

    // Пустой вход — ошибка, а не отсутствие ошибок (§13). Ноль целей значит,
    // что сцена не отрисовалась, а не что на ней всё хорошо. И отдельно —
    // страница ошибки Chrome: она НЕ пустая, кнопки на ней есть, поэтому
    // проверка по числу целей её пропускает.
    if (res.страницаОшибки || !String(res.адрес).startsWith(BASE)) {
      пустые.push(`${сцена.имя}: страница не открылась, адрес ${res.адрес}`);
      continue;
    }
    if (/НЕ ГОТОВЫ/.test(m.карты || '')) {
      пустые.push(`${сцена.имя}: ${m.карты} — замер до готовности карты даёт ложный дефект`);
      continue;
    }
    if (n === 0 || /НЕ УДАЛАСЬ/.test(m.подготовка)) {
      пустые.push(`${сцена.имя} (целей ${n}, подготовка: ${m.подготовка})`);
      continue;
    }
    // Недобор: сцена отрисовалась, но не вся. Ни ноль целей, ни страница
    // ошибки этого не ловят — а «148/148» при четырёх целях из тридцати
    // восьми читается как доказательство полноты.
    const надо = сцена.минимум ? сцена.минимум(ожид) : 1;
    if (n < надо) {
      недобор.push(`${сцена.имя}: целей ${n}, ожидалось не меньше ${надо} — сцена отрисовалась не полностью`);
      continue;
    }
    сценОК++;

    console.log(`  ${сцена.имя.padEnd(20)} целей ${String(n).padStart(3)} (≥${String(надо).padStart(3)}) · CSS ${m.css[0]}×${m.css[1]} · DPR ${m.dpr}`
      + ` → физ. ${m.физически[0]}×${m.физически[1]} · --ui-scale ${m.uiScale} · шрифты ${m.шрифты}`
      + ` · ${m.видимость} · ход ${m.ход}px в ${m.положений} положениях`
      + (m.карты && m.карты !== 'карт нет' ? ` · ${m.карты}` : ''));

    for (const k of (res.контрасты || [])) {
      контрасты.push({ сцена: сцена.имя, ...k,
        ключ: `${сцена.имя} | контраст | ${k.текст} | ${k.цвет} на ${k.фон}` });
    }

    for (const ц of res.цели) {
      const р = разобрать(ц);
      if (!р) continue;
      const запись = { сцена: сцена.имя, ...ц, вид: р.вид, ключ: ключ(сцена.имя, ц, р.вид) };
      (р.дефект ? находки : законные).push(запись);
    }
  }
  убить();
  стопСервер();

  console.log(`\nпроверено: ${целейВсего}/${целейВсего} целей на ${сценОК}/${SCENES.length} сценах`);

  if (САМОПРОВЕРКА) {
    // Замер шёл заведомо рано. Признак обязан был увидеть неготовую карту
    // хотя бы на одной сцене; если все «готовы за 0 мс» — он не умеет
    // видеть окно, и в обычном прогоне его молчание ничего не значит.
    const сКартами = сСостоянием.filter(x => x.карты && x.карты !== 'карт нет');
    console.log('\n── самопроверка признака готовности карты ──');
    for (const x of сКартами) console.log(`   ${x.сцена}: ${x.карты}`);
    if (!сКартами.length) {
      console.error('\n✗ самопроверка бессмысленна: ни на одной сцене нет карт');
      process.exit(1);
    }
    for (const x of сКартами) {
      if (x.рано) console.log(`   ${x.сцена}: в момент появления — ${x.рано.готова ? 'уже готова' : 'НЕ готова'} (transform ${x.рано.transform})`);
    }
    const поймали = сКартами.some(x => /НЕ ГОТОВЫ/.test(x.карты) || (x.рано && !x.рано.готова));
    if (!поймали) {
      console.error('\n✗ признак не увидел раннего состояния: все карты «готовы» сразу.');
      console.error('  Значит в обычном прогоне его молчание ничего не доказывает.');
      process.exit(1);
    }
    console.log('\n✓ признак ловит неготовую карту — в обычном прогоне ожидание осмысленно');
    return;
  }

  if (пустые.length || недобор.length) {
    if (пустые.length) {
      console.error(`\n✗ сцены без целей — они не отрисовались, а не «прошли»:`);
      for (const p of пустые) console.error(`    ${p}`);
    }
    if (недобор.length) {
      console.error(`\n✗ недобор целей — сцена открылась, но отрисовалась не вся:`);
      for (const p of недобор) console.error(`    ${p}`);
    }
    process.exit(1);
  }

  // Считаем по ключу: одинаковых целей бывает несколько.
  const счёт = new Map();
  for (const f of [...находки, ...контрасты]) счёт.set(f.ключ, (счёт.get(f.ключ) || 0) + 1);

  if (обновить) {
    const known = [];
    for (const [k, n] of счёт) {
      const обр = [...находки, ...контрасты].find(f => f.ключ === k);
      known.push(обр.отношение != null
        ? { ключ: k, причина: `контраст ${обр.отношение} при норме ${обр.порог}`, число: n,
            кегль: обр.кегль, цвет: обр.цвет, фон: обр.фон }
        : { ключ: k, причина: обр.вид, число: n, бокс: обр.бокс, хит: обр.хит });
    }
    await writeFile(BASELINE, JSON.stringify({
      note: 'Известные отклонения тач-целей: ключ, причина и ЧИСЛО таких целей. '
          + 'Падаем на приросте — и по новому ключу, и по выросшему числу.',
      known,
    }, null, 2) + '\n', 'utf8');
    console.log(`✓ baseline записан: ${known.length} видов — `
      + `тач-целей ${находки.length}, контраста ${контрасты.length}`);
    return;
  }

  const новые = [];
  const старые = [];
  const всеНаходки = [...находки, ...контрасты];
  for (const [k, n] of счёт) {
    const было = известно.get(k) || 0;
    const свои = всеНаходки.filter(f => f.ключ === k);
    if (n > было) {
      новые.push(...свои.slice(0, n - было).map(f => ({ ...f, прирост: `${n} против ${было} в базе` })));
      старые.push(...свои.slice(n - было));
    } else {
      старые.push(...свои);
    }
  }

  const печать = (title, arr) => {
    if (!arr.length) return;
    console.log(`\n${title} — ${arr.length}:`);
    for (const f of arr) {
      console.log(`   ${f.сцена} · ${f.подпись}`);
      console.log(`      ${f.вид} · бокс ${f.бокс.join('×')} · точки ${f.точки.join('/')} · ${f.путь}`);
    }
  };

  const новыеЦели = новые.filter(f => f.отношение == null);
  const новыеКонтрасты = новые.filter(f => f.отношение != null);
  печать('НОВЫЕ ДЕФЕКТЫ ТАЧ-ЦЕЛЕЙ', новыеЦели);
  if (новыеКонтрасты.length) {
    console.log(`\nНОВЫЕ ДЕФЕКТЫ КОНТРАСТА — ${новыеКонтрасты.length}:`);
    for (const f of новыеКонтрасты) {
      console.log(`   ${f.сцена} · «${f.текст}»`);
      console.log(`      контраст ${f.отношение} при норме ${f.порог} · кегль ${f.кегль}px · ${f.цвет} на ${f.фон} · ${f.путь}`);
    }
  }
  if (старые.length) console.log(`\nизвестных отклонений в базе: ${старые.length} (см. ${path.relative(ROOT, BASELINE)})`);
  if (всё) печать('ЗАКОННЫЕ (не дефекты)', законные);
  else if (законные.length) console.log(`законных отклонений: ${законные.length} — показать: --all`);

  if (новые.length) {
    console.error(`\n✗ новых дефектов: тач-целей ${новыеЦели.length}, контраста ${новыеКонтрасты.length}`);
    console.error('  Если это законное отклонение — зафиксируй с причиной:');
    console.error('    node scripts/check-touch-targets.mjs --update-baseline');
    process.exit(1);
  }
  console.log(`\n✓ новых дефектов нет — ни тач-целей, ни контраста (проверено контрастов: ${контрасты.length + 0} ниже нормы из всех текстовых узлов)`);
}

main().catch((e) => { console.error(e); process.exit(1); });
