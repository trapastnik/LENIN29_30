...
// ── разбор значений ────────────────────────────────────────────────────────
const REF_RE  = /^\{([a-z0-9-]+)\}$/;
const MIX_RE  = /^mix\(\s*\{([a-z0-9-]+)\}\s*,\s*\{([a-z0-9-]+)\}\s*,\s*([\d.]+)%\s*\)$/;
...
function resolve_(name, tokens, seen = []) {
...
function buildSceneCss() {
  ...
  return [
    ...
    tokensBlock,
    '',
    '/* ── Сцена НЕ участвует в удвоении ─────────────────────────────────────',
    '   Выше приехал общий хвост с @media (min-width: 2400px) → --ui-scale: 2.',
    '   Для /expo/ это лишнее: сцена масштабирует свой canvas 1920 → 3840 сама,',
    '   и ×2 поверх ×2 дало бы четырёхкратный размер. Пин идёт ПОСЛЕ media-',
    '   query, поэтому выигрывает он, а не порядок подключения файлов.',
    '',
    '   Если сцена когда-нибудь откажется от собственного масштабирования —',
    '   удалить этот блок, а не править общий хвост. */',
    ':root { --ui-scale: 1; }',
  ].join('\n');
}
