...
def ctrl(p0, p1, bend):
    """Контрольная точка дуги. Считается ЗДЕСЬ и только здесь: её просит
    и отрисовка, и расчёт стороны подписи, а две копии одной формулы
    разъезжаются молча — подпись встанет по старой дуге."""
    (x0, y0), (x1, y1) = p0, p1
...
def build(scheme, sol, src_svg):
    ...
    # СТОРОНА ПОДПИСИ ВЫБИРАЕТСЯ ОТ СТРЕЛОК, а не ставится всегда справа.
    # Замер зоны simbirsk после подъёма кегля: стрелка на Ставрополь-на-Волге
    # шла СКВОЗЬ слово «Сызрань», пересекая «з» и «р». Причина геометрическая —
    # подпись стояла справа от точки, а обе стрелки выходят из этой же точки
    # вверх-вправо, то есть правая сторона занята по построению.
    #
    # Класс дефекта другой, чем я проверяла. Я смотрела подпись × подпись
    # (узел Симбирск — Бряндино — Сенгилей, пара Свияжск — Казань), а это
    # подпись × стрелка. И при ×1.7 второй класс обострялся первым: подписи
    # выросли, стрелки нет.
    #
    # СЧИТАЕМ ЗАНЯТОСТЬ КАЖДОЙ СТОРОНЫ ОТДЕЛЬНО, А НЕ СУММУ НАПРАВЛЕНИЙ.
    # Первый вариант этой правки суммировал направления стрелок и ставил
    # подпись в противоположную сторону — и завёл НОВУЮ коллизию того же
    # класса на сентябрьской схеме: стрелка Симбирск → Бряндино прошла сквозь
    # слово «Симбирск». У Симбирска три стрелки, две входящие тянут сумму
    # влево, и подпись уехала вправо — ровно под исходящую.
    #
    # Ошибка была в устройстве, а не в числе: КОЛЛИЗИЯ ЭТО МАКСИМУМ,
    # А НЕ СРЕДНЕЕ. Одна стрелка, идущая в сторону подписи, портит её,
    # сколько бы стрелок ни шло в другую сторону; усреднение их взаимно
    # гасит и объявляет сторону свободной.
    #
    # Считаем по КАСАТЕЛЬНОЙ ДУГИ У САМОЙ ТОЧКИ, а не по хорде: дуга
    # наступления у конца выпрямляется, и её последний участок идёт не туда,
    # куда хорда. Для квадратичной Безье касательная у конца смотрит
    # на контрольную точку — она же и есть направление, в котором линия
    # уходит от города.
    #
    # И сторон у одной стрелки может быть ДВЕ: диагональ мешает и подписи
    # сбоку, и подписи снизу. Первая версия присваивала ровно одну сторону
    # по большей проекции — и объявила низ у Симбирска свободным, хотя дуга
    # от Инзы шла по нему. Сектор 55° занимает обе.
    SECT = 0.57  # cos 55°
    DIRS = {"right": (1, 0), "left": (-1, 0), "bottom": (0, 1), "top": (0, -1)}
    busy = {n: {} for n in pts}
    for a in scheme["arrows"]:
        p0, p1, bend = pts[a["from"]], pts[a["to"]], a["bend"]
        cx, cy = ctrl(p0, p1, bend)
        for end, (tx, ty) in ((a["from"], (cx - p0[0], cy - p0[1])),
                              (a["to"], (cx - p1[0], cy - p1[1]))):
            h = (tx * tx + ty * ty) ** 0.5 or 1.0
            for side, (sx, sy) in DIRS.items():
                if (tx * sx + ty * sy) / h > SECT:
                    # Запоминаем наклон: он нужен, чтобы посчитать отступ,
                    # когда свободной стороны не осталось.
                    slope = abs(ty / tx) if tx else 99.0
                    busy[end][side] = max(busy[end].get(side, 0.0), slope)

    fs = 3.333 * k * LABEL_BOOST
    dots = ['<g id="city_dots">']
    labs = ['<g id="labels_cities">']
    for n, (x, y) in pts.items():
        side = next((s for s in ("right", "left", "bottom", "top")
                     if s not in busy[n]), None)
        r = (1.1 if n == "Симбирск" else 0.8) * k * LABEL_BOOST
        dots.append(f'  <circle class="cls-33" cx="{x:.2f}" cy="{y:.2f}" '
                    f'r="{r:.3f}"/>')
        off = 1.6 * k * LABEL_BOOST
        # По вертикали подпись центрируется по точке, по горизонтали
        # отодвигается от кружка — иначе она садится на него.
        anchor, lx, ly = "", x + off, y - 1.2 * k * LABEL_BOOST
        if side == "left":
            anchor, lx = ' text-anchor="end"', x - off
        elif side in ("bottom", "top", None):
            # ВСЕ СТОРОНЫ ЗАНЯТЫ — так у Симбирска в сентябре: линия проходит
            # через город насквозь плюс вход с севера. Тогда подпись уводится
            # под точку на расстояние, БОЛЬШЕЕ, чем линия успевает опуститься
            # на полуширине слова. Отступ считается от наклона, а не берётся
            # числом: числом он был 0.68 ед. при ширине слова 3.6 — линия под
            # 41° уходила на 1.58 и шла ровно по буквам.
            slope = max(busy[n].values()) if busy[n] else 0.0
            halfw = 0.26 * fs * len(n)          # Times ≈ 0.52 em на символ
            drop = min(slope, 1.2) * halfw + 0.9 * fs
            anchor, lx = ' text-anchor="middle"', x
            ly = y - drop if side == "top" else y + drop
        labs.append(f'  <text class="cls-33" x="{lx:.2f}" '
                    f'y="{ly:.2f}"{anchor} '
                    f'font-family="\'TimesNewRomanPSMT\'" '
                    f'font-size="{fs:.3f}px">{n}</text>')
    dots.append("</g>")
    labs.append("</g>")
