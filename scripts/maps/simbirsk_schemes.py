...
def build(scheme, sol, src_svg):
    ...
    #
    # ⚠️ ПРАВИЛО РАБОТАЕТ В ОБЕ СТОРОНЫ, и второй раз оно ударило обратным
    # знаком. Домножение на k возвращает пропорции ИСХОДНОЙ карты — но
    # исходная рассчитана на кадр 302 во весь экран, а схема живёт в колонке
    # лонгрида шириной 1032. Кегль, нормальный там, здесь оказался 11.1 px
    # при пороге 16: втрое мельче нормы вместо втрое крупнее.
    #
    # То есть «домножь на отношение кадров» — только половина правила.
    # Вторая половина: отношение кадров сохраняет пропорцию, а не годность.
    # Годность проверяется В ТОМ КОНТЕЙНЕРЕ, ГДЕ ПОКАЗЫВАЕТСЯ, и не автором
    # фрагмента: у меня кадр демо-страницы, у потребителя колонка статьи.
    # Оба раза дефект нашла отрисовка, а не расчёт.
    k = vw / 302.0

    # КЕГЛЬ ПОДПИСЕЙ ПОДНЯТ. Замер зоны simbirsk на живой странице лонгрида:
    # подпись города 11.1 px при 1920, тогда как порог design для дистанции
    # 70 см — --fs-4, 16 px, «ниже не опускаться». Она со своей стороны уже
    # выжала что могла: подняла бокс с 700 до 1000, при 700 подписи были
    # 7.5 px и 40 % колонки уходило в пустую бумагу.
    #
    # Множитель считан двумя способами и разошёлся: по её замеру нужно ×1.44,
    # по моему расчёту от единиц SVG — ×1.63. Расхождение объяснимо, она мерит
    # в колонке 1032, а схема вписывается по высоте и рисуется в 892.
    # Беру 1.7, чтобы порог брался с запасом при более тесной отрисовке.
    #
    # Точки городов поднимаются ТЕМ ЖЕ множителем, и это не «за компанию».
    # Сначала я поднял только подписи — и на снимке кружки стали крошками
    # рядом с 17-пиксельным текстом. Соотношение точки и подписи было
    # унаследовано от поволжской карты, то есть намеренное; подняв одну
    # сторону, я сломал его и завёл новую диспропорцию вместо старой.
    # Один множитель на оба — восстановление пропорции, а не решение о ней.
    LABEL_BOOST = 1.7
    ...
    for n, (x, y) in pts.items():
        r = (1.1 if n == "Симбирск" else 0.8) * k * LABEL_BOOST
        dots.append(f'  <circle class="cls-33" cx="{x:.2f}" cy="{y:.2f}" '
                    f'r="{r:.3f}"/>')
        labs.append(f'  <text class="cls-33" x="{x + 1.6 * k * LABEL_BOOST:.2f}" '
                    f'y="{y - 1.2 * k * LABEL_BOOST:.2f}" '
                    f'font-family="\'TimesNewRomanPSMT\'" '
                    f'font-size="{3.333 * k * LABEL_BOOST:.3f}px">{n}</text>')
    dots.append("</g>")
    labs.append("</g>")
