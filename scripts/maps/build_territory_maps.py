...
def main():
    ...
    print()
    # ФОРМУЛИРОВКА СЧЁТЧИКА. Раньше здесь стояло «территорий показывается»,
    # и это приглашало прочитать не то: скрипт знает только про СБОРКУ в карту,
    # а доезд до карточки зависит от полей, которых у него нет и которые
    # заполняет другая зона. Оркестратор прочёл «показывается» как «видно
    # посетителю» — и был прав, что так и читается.
    #
    # Это то же правило, которое я в тот же день записала про кегль: замер
    # верен в своём контексте и молчит про чужой. Счётчик обязан называть
    # то, что действительно посчитал.
    print(f"территорий СОБРАНО В КАРТЫ: {len(shown)}/{len(with_geom)}")
    if len(shown) != len(with_geom):
        miss = sorted({i['id'] for i in with_geom} - shown)
        print(f"   не собрано: {', '.join(miss)}")
    print("   доезд до карточки здесь НЕ проверяется: она читает map_id "
          "и initial_layers в справке, их ставит зона content")
    for e in errors:
