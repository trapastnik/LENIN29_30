...
# ── редакторский хвост в подписи ───────────────────────────────────────────
#
# Заказчик дописывает пояснения составителю прямо в аннотацию: «(см. литература
# в конце)», «(Уместно использовать в части про 1919 г.)». Это указание нам,
# а не подпись посетителю, и её место в source_note_ru.
#
# Точка ПОСЛЕ закрывающей скобки — не мелочь: на ней спотыкалась прежняя версия
# правила, и «Карта-схема Симбирской операции, сентябрь 1918 г. (см. литература
# последний лист).» уезжала на экран целиком. Нашла зона maps, подтвердили
# content и оркестратор на данных.
TAIL_RE = re.compile(r"\s*\(([^()]*)\)\s*[.,;]?\s*$")

# Скобка в конце — ещё не пометка, и одной позиции мало. «Советский плакат
# периода гражданской войны (1919 г.).» — часть НАЗВАНИЯ, срезать её нельзя,
# и потеря была бы молчаливой. Такой случай в источнике уже есть (раздел 11).
#
# Отличаем по тому, что пояснение составителя идёт ОТДЕЛЬНОЙ фразой — после
...
def _media_slot(n: int, caption: str | None, url: str | None = None) -> dict:
    ...
    if caption:
        ...
        if t:
            ...
            head = caption[: t.start()]
            # Режем консервативно: либо скобка сама называет себя пометкой,
            # либо стоит отдельной фразой после законченного предложения.
            # Иначе оставляем — лучше лишняя скобка в подписи, чем срезанное
            # название: первое видно, второе нет.
            if EDITORIAL_RE.search(inner) or SENTENCE_END_RE.search(head):
                note = inner
                caption = head.strip()
    return {
        "n": n,
...
def check_no_editorial(sections: list[dict]) -> None:
    """Редакторская пометка, пережившая вырезание, роняет импорт.

    Вырезание выше намеренно консервативно и потому может промахнуться:
    заказчик напишет «источник см. в перечне литературы», шаблон не совпадёт,
    и пометка тихо доедет до экрана. Несработавшее вырезание не видно вообще —
    поэтому вторым рубежом стоит проверка, и она громкая.

    Разделение ролей: вырезание отвечает за «не испортить хорошее»,
    проверка — за «не пропустить плохое». Поодиночке каждое половинчато.

    source_note_ru НЕ проверяем: там пометке и место, это её законное поле.
    """
    bad = []
    for sec in sections:
        for m in sec["media"]:
            for field in ("caption_ru", "inv_ru"):
                v = m.get(field)
                if v and EDITORIAL_RE.search(v):
                    bad.append(
                        f"  раздел {sec['n']}, медиа {m['n']}, {field}: "
                        f"«{EDITORIAL_RE.search(v).group(0)}» — в подписи"
                    )
    if bad:
        raise SystemExit(
            "редакторская пометка осталась в подписи:\n"
            + "\n".join(bad)
            + "\n\nЭто указание составителю, а не текст посетителю — его место "
              "в source_note_ru. Либо допиши формулировку в EDITORIAL_RE, "
              "либо разбери случай руками через simbirsk.patch.json."
        )
